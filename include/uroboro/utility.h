...
namespace uroboro {
	...
	inline void print_vec(vec<N> vec) {
		std::cout << "(";
	...
	inline void print_mat(mat<N, K> mat) {
	...
	inline void print_complex(complex z) {
	...
	inline void print_complex_alg(complex z) {
		std::cout << "(" << z.a << ", " << z.b << ")" << std::endl;
	}

	inline void print_quat(quat q) {
		std::cout << "(" << q.a << ", " << q.v[0] << ", " <<
				q.v[1] << ", " << q.v[2] << ")" << std::endl;
	}

	inline void print_vec_buff(const vec_buff& v) {
	...
	inline void print_sample_stats(const vec_buff& X) {
	...
	inline void print_population_stats(const vec_buff& X) {
		std::cout << "N = " << X.size() << std::endl;
		std::cout << "Mean: " << mean(X) << std::endl;
